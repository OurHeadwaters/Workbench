...
## A First Draft · Assembled from the Content Harvest

*Draft v1 · Assembled 2026-05-06 from codetry-book/harvest.md*
*All 59 audited harvest passages placed in reading order, with transitional prose.*
*Status: UNEDITED — return to founder before sharing externally.*

---

> Per-section editorial notes are appended to each chapter below.
> Per-chapter files also exist in `codetry-book/drafts/` for easier working review.
...
That is where this work begins. Not in a boardroom, not in a grant application. On a deck in northwestern Ontario, watching the sky.
...
The hands were always in it. Before the plans, before the software, before any of the names — there were the jars.
...
That pride is load-bearing. It is not nostalgia. It is a position: that abundance is local, that preservation is a skill, that the season has something to say if you know how to listen. The Green Salt is the clearest proof of this.
...
The knowledge built around that practice — the seasonal rhythms, the community logistics, the economics of keeping margin local — could not stay in jars forever.
...
The icon is a ship. Not a fortress. Not a temple. A vessel — trim, seaworthy, ready to leave the known shore. AI is the new territory: vast, unmapped, alive with both peril and possibility. Literate programming is only the first small sail on that horizon, a tiny blip of clarity in an ocean of code. It still lacks shape. It still needs hands — many hands — practicing, refining, grounding it in reality.
...
You don't understand a food system by reading about it. You understand it by watching who eats and who doesn't, who owns the till and who doesn't, where the margin goes at the end of the day.

Here is what you see when you watch long enough.
...
A family of four spends $1,680 a month on groceries in a northern community. The same basket costs $1,000 down south. Of every federal grocery help dollar that flows in, only 58¢ reaches the shelf. The store keeps the rest. Between $1.6 and $2.0 million in grocery spend leaves the community every year — to Winnipeg, or to the one store in town.

These are not projections. They are the numbers a practitioner finds when she sits down and does the math on a specific community.
...
The store has to keep working when people don't show up. That is not a design preference — it is a northern reality. Hunting season comes. Bad weather comes. Somebody can't make it in. The store opens anyway, or it doesn't.

Here is the answer that works at this scale:
...
The contract that holds this together is not complicated. Three line items. One team that gets it done.
...
The cockpit makes three promises to the community and to the contractor alike.

**A system, not a person.** The same screens survive every retirement. Same till. Same books. Same producer cycle.
...
This is the work. Now here is how the work is done.

Every engagement follows the same six phases, in order, with open numbers at every stage.
...
It is worth saying what this plan is not.

Not a report that sits on a shelf.
Not a proposal that needs another proposal to proceed.
Not a template from a southern consulting firm.
Not contingent on an ongoing retainer to keep working.

---

The platform follows governance — not the other way around.

Read your bylaws, map your member categories, identify what needs to be digital and what should stay paper. Then build. Built in the open — you can see it working at every stage.

The tools that emerge from that process are built around how your operation actually works. Changed when your operation changes.

---

The same principle holds in other rooms.

In a residential care home, the clipboard doesn't know what a good shift looks like. The staff already do. They have always known. The problem is the tool — the clipboard, the shared Excel file, the sticky notes on the nursing station — has never matched what the staff actually do.

Bright Side replaces the clipboard, the shared Excel file, and the sticky notes on the nursing station. It is a mobile-first staff tool for residential care — built so an untrained person can pick it up mid-shift and not break anything. It doesn't ask staff to change how they work. It asks them to record what they're already doing, in the language they already use.

---

Here is how the builder describes this tool, and the disclosure is worth including in full:

I built this because I've watched care home staff work around tools that weren't made for them. A 30-minute call is enough to show you what it does and whether it fits your floor. No demo environment, no slide deck — I'll walk you through the actual tool on an actual device.

No obligation. If it's not the right fit, I'll say so.

---

Across all of this work — the store plan, the co-op platform, the care home tool — the way an engagement begins is the same.
...
Now look at the practitioner's own economics.

One practitioner, working in sequence, with a clear map of what generates margin and what doesn't.

**Community contracts** are the bridge. Active relationships, clear scope, money in motion. This is the work that funds time to build everything else. Do not let the longer-horizon thinking crowd out the next billable hour.

**Salts and syrup** — Parr's Jars — is the only cash-positive stream with no debt load. The June farmers market is a forcing function: production must be confirmed before the first stall date, not after. And the market is not just a product channel. It is the highest-density local audience you'll have all season. Have a one-liner ready for what you do beyond jars.

**Start9 privacy servers** are low friction to start, no compliance path required, cash on delivery. Every Start9 buyer is a warm consulting lead. Treat setup calls as discovery sessions, not transactions.

---

And underneath all of it, the practitioner's own discipline — the one that makes the arithmetic work:

Where every dollar goes, in plain English.

Debt-attack mode: draw only $4,000 a month from the business. The rest is forgone and stays as business surplus — all of which goes to debt. The tithe, $400, is the first claim on the draw. Take-home is $3,600.
...
There is a moment in every community project when the work stops being a set of tasks and becomes a system. Something clicks into place. The language the people in the room use starts doing structural work — holding the shape of what's being built even when the builder isn't in the room.

That is the moment this chapter is about.

It begins with a simple claim. It is not a rhetorical claim. It is a practical one.

---

The words you use to describe your economy determine what your economy can become.
...
This is not a new problem. It is an old one, moving faster now.
...
A vocabulary. Not a framework, not a methodology, not a strategic plan. A vocabulary — the specific, precise, weight-tested words that a community needs to run its own economy without importing someone else's assumptions along with the terminology.

This handbook is that vocabulary. It was built in Headwaters, a small constellation of economic systems serving northwestern Ontario — food, money, knowledge, emergency preparedness, land. The words here emerged from practice: from the specific moment when the wrong word caused a real problem, and the right word had to be found. They have been tested in the field, rejected when they didn't hold, and revised when the context changed.
...
There is a longer tradition this practice sits inside. Domain-Driven Design says the language outside the code should match the language inside the code. Conway's Law says your system will mirror the shape of the team that built it. Both are useful.

Codetry agrees that the language outside the code should be the language inside the code, and then asks a harder question: *whose language?* Domain-Driven Design typically lands on the domain expert — the analyst, the consultant, the senior engineer who has just spent a week in workshops tidying the vocabulary up. Codetry insists the noun must come from the community itself, in the form the community already uses it, before any tidying.
...
Before the handbook, before the naming discipline, there was the comparison. It is worth stating once, plainly.

Literate programming makes the reasoning the source. Codetry makes the metaphor the source.

Both are don't-trust-verify moves — show the work where the work actually does the work. But they differ at the root. Codetry is the practice of building software whose primary load-bearing material is metaphor. The naming is the architecture; the code is the medium that makes the metaphor real, clickable, and runnable.

The unit of care in literate programming is the explanation. The unit of care in codetry is the name.

Where the truth lives in literate programming: in the narrative woven between the code. Where the truth lives in codetry: in the metaphor. The chosen noun carries the constraint; the schema, the UI, and the verbs of the app follow from it.
...
The clearest proof of this is also the simplest one:

Buckets. Envelope categories. You can only pour from one to another, never summon water from nothing. Rename to *Categories* and the UI starts quietly suggesting balances can grow by clicking.
...
When we say *Saltbox Zone Zero*, we don't mean it as a label. The name is the design spec. It carries that the household is Zone Zero. It carries that this is the saltbox-house itself. And it carries that everything else in the system flows from this centre.
...
Here is what to bring with you, into the next room of your day.
...
The full six-word vocabulary of the Standby: *the Standby* (umbrella and always-on state), *a call* (a specific active event), *the watch* (active-monitoring posture), *standby stock* (the always-on reserves), *the debrief* (after-action synthesis), *centralized disruption* (the class of event). Four rungs on the severity ladder: advisory / standby / active / standdown.

The principle the Standby introduced to the constellation: **the name has to hold both states.** When a system has both a slow side and a fast side, the name has to do both jobs in one word, or the system will fork into two systems with two cultures.
...
Elsewhere in the constellation, the same discipline keeps proving itself.

In regenerative beef production — the highest-leverage chokepoint in northwestern Ontario is not pasture and not customers, it is abattoir capacity. When the centerpiece of the booking tool was renamed from *cut sheet management* to *per-customer share split*, the schema, the success metrics, and the build sequence all changed in one move. The name rewrote the system without touching a line of code.

And from a cattle rancher named Karen, one of the sharpest demonstrations of the discipline on record: when she named her paper ledger as a working artifact — not a legacy system to be replaced but the actual working record of her operation — the naming removed a feature from the build. The booking calendar was cut from version 0.1 because the ledger made it unnecessary. *Don't fix my book.* Her line. Two codetry tone-lines followed: *failures land on the tool, not on her* / *make it easier for Karen to say no, not just easier to say yes.* The name as protection, not just as architecture.
...
That sentence was written before any of the tools existed, before any of the plans were filed, before any community had seen the store operating manual or the cockpit or the handbook. It was written in the posture that made all of it possible: not urgency, not panic, not the breathless pitch — stillness. The kind of stillness that comes from knowing the dam holds.
...
What comes next is not a deployment. It is a launch — and launches require a clear sense of who is on the vessel.
...
There is a different model of practitioner in the world — the one who takes risk, reads local information, runs a P&L, and then compounds. The disciplines agree at the beginning of that sentence. They part at the end.

The codetry practitioner takes personal risk, reads the local information, runs a P&L. So far the disciplines agree. Then they part ways: the practitioner is paid to write the method down so the community can run the work without them.
...
The work described in this book is one constellation, drawn in one corner of northwestern Ontario. It is not a template. Other constellations will be drawn in the grammar of other lands.
...
Before the meeting, before the pitch, before any presentation — the invitation stands, and it is the same invitation that has always stood:
...
**Harvest coverage:** All 59 audited passages from `codetry-book/harvest.md` are placed or accounted for in this draft. Passages not in the main chapter bodies are flagged in per-chapter editorial notes (see per-section files in `codetry-book/drafts/`).
...
- `codetry-book/drafts/02-watching-the-beavers.md` — first draft with editorial questions for founder
...
2. ~~Real names Sam, Jess, and Gilles in Chapter 2 — pseudonymize or confirm consent?~~ — **Closed (partial).** Decision: pseudonymize. Names changed in chapter file (Gilles → René; Sam and Jess → Chris and Morgan). This combined draft still carries the original names in Ch2 body text — a reconciliation pass on Ch2 prose in this file is still needed.
