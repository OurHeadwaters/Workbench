...
The full six-word vocabulary of the Standby, in brief: *the Standby* (umbrella and always-on state), *a call* (a specific active event), *the watch* (active-monitoring posture), *standby stock* (the always-on reserves), *the debrief* (after-action synthesis), *centralized disruption* (the class of event). Four rungs on the severity ladder: advisory / standby / active / standdown.
...
**Narrative arc:**
<<<<<<< HEAD
The chapter follows this arc: bridge from Chapter 3 (the definition is now established — this chapter shows it standing up) → the saltbox principle (form IS function; change the name, change the structure) → the hempcrete metaphor (infill, not facade; the name is inside the wall, not bolted to the outside) → the governance stake (codetry protects the substrate for self-governance; if words drift, governance does too) → naming that changes belief before the schema opens (Watershed vs. Budget; The Siphon) → the first structural test: both-states (The Standby — one name, two tempos; slow side and fast side; the two rejected candidates) → the second structural test: both-sides (The Gate — one name, two contexts; bright side and massity; what crosses and what refuses) → the refused outcome in practice (Parr's Jars smoked salt — a real crossing that failed) → the refused callout (first-class outcome, not failure) → closing bridge ("it is a quiet discipline") → the vocabulary (the named terms of the discipline, in a compressed reference block) → close (the vocabulary is the architecture).

**Passages included from harvest.md Section 4:**
4-A (what codetry is — opening prose; folded into chapter as background tone rather than quoted), 4-C (the thesis — governance stake), 4-D (the saltbox principle — narration, exercise held back), 4-E (the hempcrete metaphor — full narration + infill passage + one-sentence callout), 4-F (both-states — narration), 4-G (The Standby — narration), 4-H (both-sides — narration), 4-I (The Gate — narration), 4-J (vocabulary sheet — term definitions, used verbatim as the vocabulary reference block), 4-K (the refused callout), 4-R (Gate Refused — Parr's Jars, the real crossing that failed).

**Passages held back:**
4-B (single-sentence definition "naming IS architecture") — already landed in Chapter 3; repeating verbatim here would feel like a retreat. The definition is assumed and built from rather than re-stated.
4-L (The Standby full role prose from deleted working doc) — the six-word vocabulary and four-rung severity ladder are rich working-doc detail; the chapter takes the narrative version (4-G) rather than the operational prose version, because the narrative is what a reader can follow. 4-L content is recoverable if the founder wants a sidebar or appendix.
4-M (The Gate full role prose from deleted working doc) — same reasoning as 4-L. The eight-word vocabulary and four-rung severity ladder are held back; the chapter takes 4-I. Recoverable for a sidebar.
4-N (Regen Revolution) — the "don't fix my book" line and the per-customer share split are strong examples of metaphor-as-architecture, but they belong to a different community context (regen beef, Karen's abattoir booking system). Held back to avoid diluting the Headwaters focus; would be strong material for a worked-examples appendix.
4-O (Community Knowledge Hub) — same reasoning as 4-N; excellent examples but a different community context. "Today I…" verbs as routing primitive and the Treasurer Calm journey would be strong appendix material.
4-P (constellation manifest principle) — the both-states / both-sides labels are folded into the vocabulary block via 4-J rather than quoted separately; 4-P as a standalone is redundant given 4-J.
4-D exercise ("walk through your home and pick the one room") — held back; the exercise form suits the handbook narration format but interrupts the chapter flow. Worth restoring if the founder wants exercises embedded in the book.

**Ordering decisions:**
- The saltbox (4-D) opens because it is the most concrete image of what "naming is architecture" actually looks like, and it is the name-closest-to-the-reader: a room, a house, a wall.
- Hempcrete (4-E) follows the saltbox because it deepens the architectural claim from "weight-bearing" to "composite and breathing" — it moves from the structural to the systemic.
- The governance stake (4-C) comes after the two architectural metaphors, not before — the metaphors have to be felt before the governance claim lands. Reversed from abstract → concrete; here it is concrete → stakes.
- Watershed/Siphon (4-Q) arrives as a quick, name-level demonstration before the structural tests, showing that naming changes belief without a single schema change.
- Both-states (4-F + 4-G) before both-sides (4-H + 4-I) because The Standby is temporally simpler — one system, two speeds — before The Gate, which requires understanding "bright side" and "massity" as new vocabulary.
- Parr's Jars (4-R) follows The Gate narration immediately, as a real-world demonstration of the refused outcome — theory, then real failure, in that order.
- The refused callout (4-K) follows the Parr's Jars example rather than preceding it — the principle lands harder after the story.
- The vocabulary block (4-J) closes the chapter as a reference the practitioner can return to, not as a glossary the reader must absorb before the chapter starts.

**Bridging copy added (not from harvest):**
- Opening ("Chapter 3 ended with a definition. This chapter shows what the definition looks like when it is standing up.")
- "The reason any of this matters beyond aesthetics is governance." (transition into 4-C)
- "Here is what that control looks like in practice, before a single schema is opened." (transition into 4-Q)
- "The discipline has two structural tests for names." (introduces the both-states / both-sides arc)
- "The constellation's first non-zone primitive demonstrates the test." (transition into The Standby)
- "The second structural test is both-sides." (transition from The Standby to The Gate)
- "The constellation's second non-zone primitive demonstrates the test." (parallel to The Standby transition)
- "The *refused* outcome is not a failure. It is a discipline." (opens the Parr's Jars section)
- "It is a quiet discipline. Most of it is naming." (closes the main narrative and opens the vocabulary block — drawn from 4-A but newly positioned)
- "Here is the vocabulary the discipline itself uses." (introduces the vocabulary block)
- Closing paragraph ("The vocabulary is not decoration. It is the architecture.")

**Decisions for the founder:**
1. **The vocabulary block placement:** The 4-J vocabulary is currently placed at the end of the chapter as a reference block, after the narrative. The founder may prefer it earlier — as a glossary that frontloads the terms before the reader encounters them in the narrative. If so, it would move to after the opening saltbox section, with a note like "Before the chapter continues, here are the named terms." Current placement assumes readers prefer to earn the vocabulary through story.
2. **The saltbox exercise (4-D):** The handbook narration ends with an exercise for the reader ("walk through your home and pick the one room that does the most jobs"). This is held back here because the book format reads differently from the handbook. If the founder wants the book to carry exercises — as a practitioner workbook as well as a narrative — the exercise could be restored at the end of the saltbox section. Decision: workbook or narrative?
3. **Watershed/Siphon placement (4-Q):** Currently used as a quick name-level demonstration between the governance stake and the structural tests. It could alternatively open the chapter — "Here is the simplest version of the claim" — before the architectural metaphors arrive. Placing it early would make the chapter more immediately accessible to a non-technical reader. Current placement assumes the architectural metaphors are the right opening for a chapter called "Codetry as Architecture."
4. **4-N and 4-O (Regen Revolution and Knowledge Hub):** Both contain excellent examples of metaphor-as-architecture from different community contexts. Currently held back to keep the chapter's examples anchored in the Headwaters constellation. If the founder wants this chapter to demonstrate the discipline's portability beyond one context, 4-N and 4-O could be added as a section between the Gate and the vocabulary block — "The same moves appear elsewhere."
...
**Editorial decision needed — deleted-artifact material (4-L, 4-M):**
The full role prose for the Standby (4-L) and the Gate (4-M) from the deleted working doc is condensed in this draft into the constellation-vocabulary notes and closing field examples. The full verbatim passages are long and dense; including them in full would shift the chapter's register toward reference manual. Founder should decide whether to restore them in full as a Deep Dive appendix or keep them condensed as they appear here.
