...
Chapter 3 ended with a definition. This chapter shows what the definition looks like when it is standing up.

The naming is the architecture. The code is the medium that makes the metaphor real, clickable, and runnable. That is the claim. The rest of this chapter is the practice.
...
*A practitioner invitation: walk through your home and pick the one room that does the most jobs. Write down — by hand, on paper — the three jobs it carries. Tape the paper to the doorframe. That paper is your first piece of codetry. The room hasn't changed. But now its name is doing the work the architect's drawing used to do.*
...
*A practitioner invitation: take one piece of mail from a regulator, a banker, or a government office. Underline every word in it your family wouldn't say at the table. Write your family's word in the margin, next to each one. Keep both. The two together are the gate.*
...
*Don't fix my book.* That line came from Jude, a practitioner running a regen beef operation in NWO. Her paper ledger was already working. It was a working artifact, not a problem to be solved. Naming it as a working artifact — taking that line seriously as a design instruction — removed a feature, the booking calendar, from v0.1. The name didn't just describe the system. It narrowed the build.

Two tone-lines followed from that naming: *failures land on the tool, not on her* / *make it easier for Jude to say no, not just easier to say yes.* The name as protection, not just as architecture.
...
In regenerative beef production, the highest-leverage chokepoint in northwestern Ontario is not pasture and not customers — it is abattoir capacity. When the centrepiece of the booking tool was renamed from *cut sheet management* to *per-customer share split*, the schema, the success metrics, and the build sequence all changed in one move. The name rewrote the system without touching a line of code.
...
**Zone** — A domain of practice inside the constellation. Six zones, numbered by proximity to the household: Zone 0 — household; the Eave — finance; Zone 2 — knowledge; Zone 3 — emergency preparedness; Zone 4 — land; Zone 5 — public. Primitives are hosted in zones but read by all zones.
...
Take these words with you. The names you carry into the next project are the first decisions you make about its structure. Make them slowly, write them down, and refuse to translate them until you are certain the translation is honest.

The only architecture that survives is the one whose names kept their promises.
