...
const BUCKETS: { slug: string; name: string; description?: string; color?: string }[] = [
  ...
  { slug: "circular-economies", name: "Circular Economies Project", description: "Research and programming on circular economy models in Northern Ontario food systems.", color: "#5A7A6B" },
  { slug: "dhs-coop-students", name: "DHS Co-op Students", description: "Dryden High School co-op student placements and programming with the food hub.", color: "#7A6B9A" },
  { slug: "draft-projects-unfunded", name: "Draft Projects — Unfunded Pipeline", description: "Project concepts and grant drafts that are not yet funded.", color: "#8A7060" },
  { slug: "field-guide-finance", name: "Field Guide Finance", description: "Financial field guides, business plans, and operational finance resources.", color: "#4A6B8A" },
  { slug: "food-bank-partnership", name: "Food Bank Partnership", description: "Partnership programming and resource sharing with local food banks.", color: "#8A4A4A" },
  { slug: "pilot-programs", name: "Pilot Programs", description: "Short-run pilot programs testing new delivery, production, or market models.", color: "#6B8A4A" },
  { slug: "processing-centre-research", name: "Processing Centre Research", description: "Feasibility and planning research for regional food processing centres.", color: "#4A7A8A" },
  { slug: "thunder-bay-distribution", name: "Thunder Bay Distribution", description: "Distribution logistics, routes, and partnerships centred on Thunder Bay.", color: "#7A4A8A" },
  { slug: "vegetable-boxes-2025", name: "Vegetable Boxes 2025", description: "2025 vegetable box program — producer commitments, pricing, and customer logistics.", color: "#5A8A4A" },
];
...
const RAW_ATTACHED = `2024_Strategic_Business_Plan_(2)_1779238383530.pdf
Operational_Plan_2024_1779238383530.docx
Vision_&_Mission_Exercise_1779238383530.docx
...
const META_BY_FILE: Record<string, { title: string; summary?: string; producerSlug?: string; subjects: string[]; buckets: string[]; contributorIndex?: number }> = {
  "2024_Strategic_Business_Plan_(2)_1779238383530.pdf": {
    title: "2024 Strategic Business Plan — 807 Food Co-op",
    summary: "Official 2024 strategic and business plan for 807 Food Co-op Inc., Dryden, ON.",
    producerSlug: "807-food-coop",
    subjects: ["policy-governance", "research-reports"],
    buckets: ["807-nwo-hub", "field-guide-finance"],
  },
  "Operational_Plan_2024_1779238383530.docx": {
    title: "Operational Plan 2024 — 807 Food Co-op",
    summary: "Working operational plan for 807 Food Co-op, covering day-to-day operations and delivery logistics for 2024.",
    producerSlug: "807-food-coop",
    subjects: ["policy-governance", "research-reports"],
    buckets: ["807-nwo-hub", "field-guide-finance"],
  },
  "Vision_&_Mission_Exercise_1779238383530.docx": {
    title: "Vision & Mission Exercise — 807 Food Co-op",
    summary: "Facilitated vision and mission exercise document for 807 Food Co-op.",
    producerSlug: "807-food-coop",
    subjects: ["policy-governance", "research-reports"],
    buckets: ["807-nwo-hub", "field-guide-finance"],
  },
...
// ---------- web-source stubs ----------
// Entries for Drive-hosted documents that cannot be downloaded without authentication.
// Seeded as web_source + needs_review so curators can retrieve and attach the file later.
// Idempotency key: title (exact match).
...
const WEB_SOURCE_STUBS: WebSourceStub[] = [
  // --- 807 Strategic Planning folder — Google Docs not available as local files ---
  {
    title: "Operational Plan Template 2024 — 807 Food Co-op",
    summary: "Reusable operational plan template, 2024 edition. Word document in the 807 Strategic Planning Drive folder. Needs curator download. Drive URL: https://drive.google.com/drive/folders/1goiXvv2QkCUFQfxh38JeaOWD2qoha6_Y",
    sourceUrl: "https://drive.google.com/file/d/1dyyf-zqL2vLhI2XQgTvhDnm5Z6lf8390/view",
    producerSlug: "807-food-coop",
    subjects: ["policy-governance", "research-reports"],
    buckets: ["807-nwo-hub", "field-guide-finance"],
    status: "needs_review",
  ...
  {
    title: "SENT 2024–2029 Strategic/Business Plan — 807 Food Co-operative",
    summary: "Sent version of the 2024–2029 strategic and business plan for 807 Food Co-operative. Google Doc in the 807 Strategic Planning Drive folder. Needs curator export. Drive URL: https://drive.google.com/drive/folders/1goiXvv2QkCUFQfxh38JeaOWD2qoha6_Y",
    sourceUrl: "https://docs.google.com/document/d/13vJ5fiI9XarM3MdbffVYLK2MeJ_gkydqtZnTTHTN2eU/view",
    producerSlug: "807-food-coop",
    subjects: ["policy-governance", "research-reports"],
    buckets: ["807-nwo-hub", "field-guide-finance"],
    status: "needs_review",
  },
  // --- Programs & Initiatives folder — root-level files ---
  {
    title: "DCFH Project — Google Doc",
    summary: "DCFH project document from the Programs & Initiatives Google Drive folder. Needs review and local file attachment. Drive URL: https://drive.google.com/drive/folders/1OZgP7p8sR7UN-BN6_OM7OrOPWqaaNKxF",
    sourceUrl: "https://drive.google.com/drive/folders/1OZgP7p8sR7UN-BN6_OM7OrOPWqaaNKxF",
    subjects: ["policy-governance", "indigenous-communities"],
    buckets: ["807-nwo-hub"],
    status: "needs_review",
  },
  {
    title: "Face Sheet Template — DHS Co-op Students",
    summary: "Face sheet template for DHS co-op student program placements. From the root level of the Programs & Initiatives Drive folder. Needs review and local file attachment.",
    sourceUrl: "https://drive.google.com/drive/folders/1OZgP7p8sR7UN-BN6_OM7OrOPWqaaNKxF",
    subjects: ["store-operations"],
    buckets: ["dhs-coop-students"],
    status: "needs_review",
  },
  {
    title: "Lori Skitt — Equipment & Services List",
    summary: "Equipment and services list compiled by Lori Skitt, related to processing centre infrastructure. From the root level of the Programs & Initiatives Drive folder. Needs review and local file attachment.",
    sourceUrl: "https://drive.google.com/drive/folders/1OZgP7p8sR7UN-BN6_OM7OrOPWqaaNKxF",
    subjects: ["kitchen-infrastructure"],
    buckets: ["processing-centre-research"],
    status: "needs_review",
  },
];
...
async function main() {
  ...
  // Clean up stale web_source stubs superseded by real file entries, or with incorrect taxonomy.
  // Deletes only entries with kind=web_source whose titles now have a real file entry (or were mis-tagged).
  // Delete web_source stubs whose titles are now covered by real file entries in META_BY_FILE.
  const STALE_WEB_SOURCE_TITLES = Object.values(META_BY_FILE).map((m) => m.title);
  const deleted = await db
    .delete(libraryEntriesTable)
    .where(
      sql`${libraryEntriesTable.kind} = 'web_source' AND ${libraryEntriesTable.title} = ANY(ARRAY[${sql.raw(STALE_WEB_SOURCE_TITLES.map((t) => `'${t.replace(/'/g, "''")}'`).join(","))}]::text[])`,
    )
    .returning({ id: libraryEntriesTable.id });
  if (deleted.length) {
    console.log(`[seed] cleanup: removed ${deleted.length} stale web_source stub(s)`);
  }
  ...
  // Seed web-source stubs (Drive-hosted docs, no local file). Idempotent by title.
  console.log(`[seed] web-source stubs (${WEB_SOURCE_STUBS.length})`);
  let stubsCreated = 0;
  let stubsSkipped = 0;
  for (const stub of WEB_SOURCE_STUBS) {
    const existing = await db
      .select()
      .from(libraryEntriesTable)
      .where(eq(libraryEntriesTable.title, stub.title))
      .limit(1);
    if (existing.length) {
      stubsSkipped++;
      continue;
    }
    const producerId = stub.producerSlug ? producerMap.get(stub.producerSlug) ?? null : null;
    const [row] = await db
      .insert(libraryEntriesTable)
      .values({
        kind: "web_source",
    ...
    const subjectIds = stub.subjects.map((s) => subjectMap.get(s)).filter((x): x is string => !!x);
    if (subjectIds.length) {
      await db
        .insert(entrySubjectsTable)
        .values(subjectIds.map((sid) => ({ entryId, subjectId: sid })))
    ...
    const bucketIds = stub.buckets.map((b) => bucketMap.get(b)).filter((x): x is string => !!x);
    if (bucketIds.length) {
      await db
        .insert(entryBucketsTable)
        .values(bucketIds.map((bid) => ({ entryId, bucketId: bid })))
  ...
  console.log(`[seed] web-source stubs done: created=${stubsCreated} skipped=${stubsSkipped}`);
