# Agents working in `artifacts/library`
...
`source = 'artifacts/library'`, then flow into `deadhead_items` when the
...
`putOnKitchenTable({ title, source: "artifacts/library" })`. **Do not**
