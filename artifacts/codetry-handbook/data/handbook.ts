...
const partVI: Part = {
  ...
  chapters: [
    ...
    },
    {
      ...
      partRoman: "VI",
      partLabel: "VI · Open Questions",
      title: "What bringing a sledgehammer to the party looks like",
      blocks: [
        {
          kind: "small",
          text: "Open question · the §6.1 problem one floor up · returned to as the answer changes.",
        },
        {
          kind: "para",
          text:
            "On 29 April 2026 the practitioner authorised a rewrite of the entire handbook into plain grade-9 language. The brief was, on its face, kind: lower the reading level so the book would land on a wider counter — the kitchen table the rest of the discipline keeps invoking. The rewrite was thorough. Every chapter body was retyped in shorter sentences with everyday verbs. A scanner was built and tightened, in three passes over the same afternoon, to keep the longest sentence under thirty-five words. A banned-vocabulary list was installed inside the same scanner: *substrate*, *primitive*, *membrane*, *load-bearing*, *cross-zone*, *vernacular*, *tokenize*, *reify*. Each banned term was replaced inline with a plainer substitute. The next morning a third pass tried to smooth the cadence the cap had introduced. The book passed every test the new infrastructure imposed. It also stopped being the book the practitioner had been writing.",
        },
        {
          kind: "para",
          text:
            "The diagnosis the chapter exists to record is short and unflattering to the practitioner who authorised the move. The eight banned words are not academic decoration on top of a craft that could just as well be described in everyday speech. They are the named primitives the craft is built on. *Load-bearing* is what §1.0 means when it says the noun carries the weight of the building; the metaphor is the discipline. *Substrate* is the word §6.1 reaches for when it has to distinguish the layer the cutter eats from the layer the author writes. *Membrane* is what §1.4 calls the gate between rooms before it earns the name *Gate*; *primitive* is the registered category of named pieces in the constellation manifest. Stripping the words and leaving the prose was not a translation. It was a removal of the foundational vocabulary the prose stood on, with a new prose laid over the gap. The new prose was lighter to read because there was less holding it up.",
        },
        {
          kind: "subhead",
          text: "Why this is the §6.1 problem one floor up.",
        },
        {
          kind: "para",
          text:
            "§6.1 names a hazard from below: the cutter shreds the noun into sub-word fragments the model can read but the human author cannot author into. §6.2 names the same hazard one floor down: the chain shreds the noun into addresses the bytecode can read but the community cannot author into. The simplification pass of 29 April was the same hazard from above: a substitution rule, applied uniformly across the corpus, shredded the named primitives into everyday synonyms the new reader could decode but the discipline could no longer be built out of. The rule was not malicious — it was kind. It was *make this readable to more people*. The result was indistinguishable in shape from the cutter and the chain. A layer that could read at one resolution forced a transformation on a layer authored at another, and the authored layer lost. That is the §6.1 frame. The chapter belongs in Part VI for the same reason §6.1 and §6.2 do: the practice has no settled answer to it yet.",
        },
        {
          kind: "subhead",
          text: "What the lesson actually says.",
        },
        {
          kind: "para",
          text:
            "The lesson is *not* that the handbook should never be readable to a non-specialist. The §1.0 line about the kitchen table still holds, and the practitioner still owes the community a book it can sit with. The lesson is that the plain-language register is a *derivative* artifact, not a *replacement* one. The academic register is where the craft's nouns are authored, defined, cross-referenced, and tested against each other. The plain-language register is what gets carried out of that workshop into the community room. Authoring the plain register first, by stripping the academic one, collapses the workshop into the room and leaves nothing in the workshop to carry anything else out of, ever again. The §4.2 rename test catches drift inside a single register. It does not catch the deeper drift introduced when one register is asked to do the work of two.",
        },
        {
          kind: "subhead",
