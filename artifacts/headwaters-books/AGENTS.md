# Agents working in `artifacts/headwaters-books`
...
`source = 'artifacts/headwaters-books'`, then flow into `deadhead_items` when the
...
`putOnKitchenTable({ title, source: "artifacts/headwaters-books" })`. **Do not**
