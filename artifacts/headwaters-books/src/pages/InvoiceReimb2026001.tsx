/**
 * Invoice REPLIT-DIGITAL-REIMB-2026-001
...
const LINE_ITEMS = [
  {
    ...
    desc: "Replit principal charges — 116 charges, Apr 17 – Jun 26 2026 (CAD card amounts from Alterna Savings statements; ref. RZSJOV- invoice series for detail)",
...
export default function InvoiceReimb2026001() {
  return (
      ...
      <div className="border border-border rounded-xl overflow-hidden print:border-black print:rounded-none shadow-sm print:shadow-none">
        ...
        <div className="px-8 py-6 grid grid-cols-1 sm:grid-cols-3 gap-6 border-b border-border print:border-black/20">
          ...
          <div className="space-y-2">
            ...
            <div>
              ...
              <p className="text-sm font-medium">{INVOICE_PERIOD}</p>
            </div>
            <div>
              <p className="text-[10px] font-mono uppercase tracking-[0.2em] text-muted-foreground">
                Payment terms
        ...
        <div className="px-8 py-4 border-b border-border print:border-black/20 bg-muted/20 print:bg-transparent">
          <p className="text-xs text-muted-foreground leading-relaxed">
            <strong className="text-foreground">Re: Reimbursement of digital development costs.</strong>{" "}
            ...
            {INVOICE_PERIOD}. All amounts are in Canadian dollars (CAD) sourced from the Alterna
            Savings credit-card statements. These costs relate exclusively to software development
            platforms, domains, AI tooling, and related digital infrastructure used for the
            Corporation's projects. No HST is charged on this reimbursement invoice — costs were
            incurred as a pass-through and are not a supply of services by the sole proprietor.
          </p>
        </div>
        ...
        <div className="px-8 pt-4 pb-0">
          ...
          {LINE_ITEMS.filter((l) => l.group === "Replit Development Platform").map((item, i) => (
            <div key={i} className="grid grid-cols-[1fr_auto] gap-4 py-1.5 border-b border-dashed border-border/50 print:border-black/10 last:border-0">
              ...
              <p className="text-sm font-mono tabular-nums text-right shrink-0 pt-0.5">{fmt(item.amount)}</p>
            </div>
          ))}
          <div className="grid grid-cols-[1fr_auto] gap-4 py-2 border-b border-border print:border-black/20 bg-muted/30 -mx-8 px-8 mt-1">
            ...
            <p className="text-sm font-mono font-semibold tabular-nums text-right">{fmt(REPLIT_SUBTOTAL)}</p>
          </div>
          ...
          <div className="grid grid-cols-[1fr_auto] gap-4 py-2 border-b border-border print:border-black/20 bg-muted/30 -mx-8 px-8 mt-1">
            ...
            <p className="text-sm font-mono font-semibold tabular-nums text-right">{fmt(OTHER_DIGITAL_SUBTOTAL)}</p>
          </div>
        </div>
        ...
        <div className="px-8 py-5 flex items-center justify-between bg-primary/5 print:bg-white print:border-t print:border-black">
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-0.5">
              Replit ({fmt(REPLIT_SUBTOTAL)}) + Other digital ({fmt(OTHER_DIGITAL_SUBTOTAL)}) · No HST
            </p>
          </div>
        ...
        <div className="px-8 py-5 border-t border-border print:border-black/20 space-y-3">
          ...
          <p className="text-xs text-muted-foreground leading-relaxed">
            ...
            <strong className="text-foreground">{fmt(INVOICE_TOTAL)} CAD</strong> to Bobbie Parr's
