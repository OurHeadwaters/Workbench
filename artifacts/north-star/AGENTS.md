# Agents working in `artifacts/north-star`
...
`source = 'artifacts/north-star'`, then flow into `deadhead_items` when the
...
`putOnKitchenTable({ title, source: "artifacts/north-star" })`. **Do not**
