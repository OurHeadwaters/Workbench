# Agents working in `artifacts/field-guide-finance`
...
`source = 'artifacts/field-guide-finance'`, then flow into `deadhead_items` when the
...
`putOnKitchenTable({ title, source: "artifacts/field-guide-finance" })`. **Do not**
