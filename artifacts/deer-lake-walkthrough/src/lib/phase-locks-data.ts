/**
 * Canonical phase-lock data.
 *
 * Single source of truth consumed by both:
 *   - src/sections/PhaseLocks.tsx   (the read surface / pitch scroll)
 *   - src/sections/PhaseLockSignoff.tsx  (the printable sign-off sheet)
 *
 * Adding or editing a phase here updates both surfaces automatically.
 */
...
export const PHASE_LOCKS: PhaseLockEntry[] = [
  ...
  },
  {
    ...
    decisions: [
      ...
      "Opening-day staffing — operator couple + Headwaters food-safety person, hour by hour.",
    ],
...
/**
 * Fixed sign-off columns for every phase gate, as named in the task spec.
 * These appear as blank-line rows on the printed sheet regardless of which
 * people happen to be in the room at each gate.
 */
