...
/**
 * Four named scenarios for the Deer Lake project calendar. Three model
 * the federal grant path (best, expected, what-if). The fourth models
 * the no-grants path: band reserve capital pays for the store, the only
 * grant in play is the LFIF window for the 807-partnership ice-road
 * truck (which doesn't gate the store, just the vehicle).
 */
...
export const SCENARIOS: Record<ScenarioId, Scenario> = {
  ...
  selfFund: {
    ...
    mode: "self-fund",
    caption:
      "Reserve capital pays for the store. Only grant: the 807 truck. Doors open spring 2027.",
    anchors: {
      contractOneStart: "2026-04-27",
      coldChainPilotStart: "2026-06-01",
      // Council vote happens earlier — there's no application package
      // to wait on, just a pilot-data + design package and a number.
      councilDecision: "2026-08-15",
      truckLfifIntake: "2026-10-15",
      // The two grant-only anchors are unused in self-fund mode but
      // stay in the type so the Anchors shape is uniform across modes.
      lfifIntake: "2026-10-15",
      iscDecision: "2027-07-15",
    },
  },
