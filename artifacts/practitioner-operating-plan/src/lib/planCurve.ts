// The 11-year plan curve from age 39 (2026) to age 50 (April 2037).
// Mirrors the standalone Annual Plan Check-in artifact verbatim so the
// retirement-outlook math, pace bucketing, and chart all read from the
// same source of truth as the original app.
//
// Anchor points:
...
//   * Owner take-home tracks roughly half of ARR.
//   * Living expenses creep with modest inflation.
...
// Pace bucketing.  Green only when at or above target; yellow 80–99%; red
// below 80%.  These thresholds are the heart of the dashboard, so be very
// strict about them.
...
/** Whole years between snapshot year and the age-50 year (2037). */
export function yearsToRetirement(snapshotYear: number): number {
  return Math.max(0, TARGET_RETIRE_YEAR - snapshotYear);
}

/**
 * Project the portfolio forward to age 50 assuming the current investing
 * rate continues unchanged and the portfolio earns `realReturn` per year
 * (default 7% real, the standard long-run U.S. equity assumption after
 * inflation).
 *
 ...
 *
 * Returns null if there are no years left to project (handled at call site).
 */
