...
export function KeyDates({ d, mode }: { d: Derived; mode: ScenarioMode }) {
  ...
  if (mode === "self-fund") {
    ...
    if (d.lfifDecision) rows.push({ label: "LFIF decision", value: fmtShort(d.lfifDecision) });
    if (d.fedNorDecision) rows.push({ label: "FedNor decision", value: fmtShort(d.fedNorDecision) });
  }
  ...
  rows.push(
    ...
    { label: "Handover to community manager", value: fmtShort(d.buildHandover) },
  );
  ...
  return (
    <section className="w-full" style={{ background: "var(--cs-bg)" }}>
      <div className="mx-auto max-w-[36rem] px-6 py-6">
        <div className="mono text-[10px] uppercase tracking-[0.22em] mb-2" style={{ color: "var(--cs-accent-warm)" }}>What falls out</div>
        ...
        <div className="rounded-xl border overflow-hidden" style={{ borderColor: "var(--cs-rule)", background: "var(--cs-paper)" }} data-testid="key-dates">
          {rows.map((r, i) => (
            <div key={r.label} className="grid grid-cols-[1fr_auto] gap-3 items-baseline px-4 py-3"
              style={{ borderTop: i === 0 ? "none" : "1px solid var(--cs-rule)", background: r.tone === "warm" ? "rgba(184,90,62,0.06)" : "transparent" }}>
              <p className="serif text-[14px] leading-tight"
                style={{ color: r.tone === "warm" ? "var(--cs-accent-warm)" : "var(--cs-text)", fontWeight: r.tone === "warm" ? 600 : 400 }}>
                {r.label}
              </p>
              <p className="mono text-[13px]"
                style={{ color: r.tone === "warm" ? "var(--cs-accent-warm)" : "var(--cs-primary)", fontWeight: r.tone === "warm" ? 600 : 500 }}>
                {r.value}
              </p>
            </div>
          ))}
