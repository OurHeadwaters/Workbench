...
import { SCENARIOS, isScenarioId, type ScenarioId } from "./scenarios";

const KEY_SAVED = "cspp:v1:saved";
const KEY_LAST_SCENARIO = "cspp:v1:lastScenario";
...
function inferMode(rawMode: unknown, scenarioId: ScenarioId | null): ScenarioMode {
...
export function loadSaved(): SavedState | null {
  try {
    ...
    const scenarioId = isScenarioId(parsed?.scenarioId) ? parsed.scenarioId : null;
    const mode = inferMode(parsed?.mode, scenarioId);
    return { scenarioId, mode, anchors, savedAt: parsed?.savedAt ?? "" };
  } catch {
    return null;
  }
}
