  ...
  return (
    ...
    >
      <div className="space-y-3">
        {ROWS.map((row) => (
          <Card key={row.label} tag={row.label} head={row.shared}>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mt-1">
              <div className="rounded-md p-3" style={{ background: "rgba(31,61,46,0.06)", border: "1px solid var(--cs-rule)" }}>
                <div className="text-[10px] uppercase tracking-[0.18em] mb-1" style={{ color: "var(--cs-accent-warm)", fontFamily: "'IBM Plex Mono', ui-monospace, monospace" }}>Store</div>
                <div className="text-[14px] leading-[1.45]" style={{ color: "var(--cs-text)" }}>{row.store}</div>
              </div>
              <div className="rounded-md p-3" style={{ background: "rgba(31,61,46,0.06)", border: "1px solid var(--cs-rule)" }}>
                <div className="text-[10px] uppercase tracking-[0.18em] mb-1" style={{ color: "var(--cs-accent-warm)", fontFamily: "'IBM Plex Mono', ui-monospace, monospace" }}>Hotel</div>
                <div className="text-[14px] leading-[1.45]" style={{ color: "var(--cs-text)" }}>{row.hotel}</div>
              </div>
            </div>
          </Card>
        ))}
      ...
      <Reveal label="Why one model and not two">
        <p>A second model means a second runbook, a second training program, a second set of indicators, a second contractor. A lesson learned in the store doesn't reach the hotel.</p>
        <p>The shared layer is what makes this affordable. The band staffs one bench that can move between buildings. The contractor runs one handover, not two.</p>
      </Reveal>

      <Reveal label="What is genuinely different per building">
        <p>The store sells perishable groceries on a margin. Cold-chain breaks show up as spoilage. The bench needs cooler and freezer ownership specifically.</p>
        <p>The hotel sells overnight stays and a small breakfast service. Cleanliness, double-bookings, and turn-over time are the failure modes.</p>
        <p>Same six functions in both. The job each function does is different.</p>
      </Reveal>
    </PageFrame>
  );
}
