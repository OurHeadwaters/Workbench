# Agents working in `artifacts/codetry-ship`
...
`source = 'artifacts/codetry-ship'`, then flow into `deadhead_items` when the
...
`putOnKitchenTable({ title, source: "artifacts/codetry-ship" })`. **Do not**
