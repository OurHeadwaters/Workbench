...
function saveRecord(note: string) {
  const now = new Date();
  const dateStr = now.toLocaleDateString("en-CA"); // YYYY-MM-DD
  const timeStr = now.toLocaleTimeString("en-CA", { hour: "2-digit", minute: "2-digit" });

  const lines: string[] = [
    "CODETRY MILL — ENGAGEMENT CLEARANCE RECORD",
    "===========================================",
    "",
    `Date:  ${dateStr}  ${timeStr}`,
    "",
    "Filter questions — all cleared",
    "-------------------------------",
  ];

  FILTER_QUESTIONS.forEach((item, i) => {
    lines.push(`[✓]  ${String(i + 1).padStart(2, "0")}  ${item.q}`);
  });

  lines.push("");
  lines.push("Status:  CLEAR TO OPEN A SCOPE");
  lines.push("");
  ...
  lines.push("---");
  lines.push("Generated by Codetry Ship · codetry.ca");

  const blob = new Blob([lines.join("\n")], { type: "text/plain;charset=utf-8" });
  const url = URL.createObjectURL(blob);
  const a = document.createElement("a");
  a.href = url;
  a.download = `mill-clearance-${dateStr}.txt`;
  a.click();
      ...
      <div className="relative z-10 mx-auto max-w-[52rem] px-6 sm:px-8 py-12 sm:py-16">
        ...
        <section className="mb-14" data-testid="mill-section-filters">
          ...
          >
              ...
              {allClear && (
                <div className="flex items-center gap-3 flex-wrap">
                  <button
                    ...
                    data-testid="mill-save-record-btn"
                  >
                    Save record ↓
                  </button>
                  <a
            ...
            {allClear && showNoteField && (
              <div
                className="px-5 pb-4 pt-0"
                style={{ borderTop: `1px solid ${ZONE2_BLUE_DIM}` }}
                data-testid="mill-save-record-panel"
              >
                <label
                  className="block font-mono text-[9px] uppercase tracking-[0.22em] mb-2 mt-4"
                  style={{ color: "rgba(244,237,224,0.38)" }}
                  htmlFor="mill-note"
                >
                  Note (optional — appears in the downloaded file)
                </label>
                <textarea
                  id="mill-note"
                  rows={3}
                  ...
                  placeholder="e.g. Engagement with Deer Lake First Nation — initial conversation with CAO, referred by …"
                  className="w-full rounded font-serif text-[13.5px] leading-[1.6] resize-none"
                  style={{
                  ...
                  data-testid="mill-note-field"
                />
                <div className="mt-3 flex justify-end">
                  <button
                    type="button"
                    onClick={() => saveRecord(note)}
                    className="font-mono text-[10px] uppercase tracking-[0.18em] px-4 py-2 rounded transition-opacity hover:opacity-80"
                    style={{
                      background: ZONE2_BLUE,
                      color: CREAM,
                    }}
                    data-testid="mill-download-btn"
                  >
                    Download record →
                  </button>
                </div>
