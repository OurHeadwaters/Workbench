...
## ✅ Current status (tested end-to-end 2026-06-17)

| Item | Status |
|------|--------|
| Webhook handler code (`stripeWebhook.ts`) | ✅ Complete — now writes tokens to `kit_tokens` DB table |
...
| End-to-end test (purchase → token → email → magic link) | ✅ Verified 2026-06-17 — token in DB, email sent, magic link returns kit content |
