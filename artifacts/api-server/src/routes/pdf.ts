...
router.get("/scope-rate-sheet.pdf", async (_req, res) => {
  ...
  try {
    ...
    const page = await browser.newPage();
    const printMarketingBase = process.env.PRINT_MARKETING_URL ?? "http://localhost:25703/print-marketing";
    ...
    res.set({
      "Content-Type": "application/pdf",
      "Content-Disposition": 'attachment; filename="headwaters-scope-rate-sheet.pdf"',
